...
abstract class BasesfGuardGroup extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|sfGuardUser[] Collection to store aggregation of sfGuardUser objects.
     */
     ...
     * @var		PropelObjectCollection
     */
    protected $sfGuardUsersScheduledForDeletion = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->sfGuardUsersScheduledForDeletion !== null) {
                if (!$this->sfGuardUsersScheduledForDeletion->isEmpty()) {
                    $pks = array();
                    $pk = $this->getPrimaryKey();
                    foreach ($this->sfGuardUsersScheduledForDeletion->getPrimaryKeys(false) as $remotePk) {
                        $pks[] = array($remotePk, $pk);
                    }
                    sfGuardUserGroupQuery::create()
                        ->filterByPrimaryKeys($pks)
                        ->delete($con);
                    $this->sfGuardUsersScheduledForDeletion = null;
                }

                foreach ($this->getsfGuardUsers() as $sfGuardUser) {
                    if ($sfGuardUser->isModified()) {
                        $sfGuardUser->save($con);
                    }
                }
            } elseif ($this->collsfGuardUsers) {
                foreach ($this->collsfGuardUsers as $sfGuardUser) {
                    if ($sfGuardUser->isModified()) {
                        $sfGuardUser->save($con);
                    }
                }
     ...
     * to the current object by way of the sf_guard_group_permission cross-reference table.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * through the sf_guard_group_permission cross reference table.
     *
     * @param  sfGuardPermission $sfGuardPermission The sfGuardGroupPermission object to relate
     * @return sfGuardGroup The current object (for fluent API support)
     */
    ...
    protected function doAddsfGuardPermission($sfGuardPermission)
    {
        ...
        $sfGuardGroupPermission->setsfGuardPermission($sfGuardPermission);
        $this->addsfGuardGroupPermission($sfGuardGroupPermission);
    }
    ...
    /**
     * Clears out the collsfGuardUsers collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
    ...
    public function clearsfGuardUsers()
    {
        ...
        $this->collsfGuardUsersPartial = null;

        return $this;
    }

    /**
     * Initializes the collsfGuardUsers collection.
     *
     * By default this just sets the collsfGuardUsers collection to an empty collection (like clearsfGuardUsers());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
    ...
    {
        $this->collsfGuardUsers = new PropelObjectCollection();
        $this->collsfGuardUsers->setModel('sfGuardUser');
    }

    /**
     * Gets a collection of sfGuardUser objects related by a many-to-many relationship
     ...
     * @param PropelPDO $con Optional connection object
     *
     * @return PropelObjectCollection|sfGuardUser[] List of sfGuardUser objects
     */
    public function getsfGuardUsers($criteria = null, PropelPDO $con = null)
    {
        if (null === $this->collsfGuardUsers || null !== $criteria) {
            if ($this->isNew() && null === $this->collsfGuardUsers) {
                // return empty collection
                $this->initsfGuardUsers();
            } else {
                $collsfGuardUsers = sfGuardUserQuery::create(null, $criteria)
                    ->filterBysfGuardGroup($this)
                    ->find($con);
                if (null !== $criteria) {
                    return $collsfGuardUsers;
                }
                $this->collsfGuardUsers = $collsfGuardUsers;
            }
        }
    ...
    /**
     * Sets a collection of sfGuardUser objects related by a many-to-many relationship
     * to the current object by way of the sf_guard_user_group cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $sfGuardUsers A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return sfGuardGroup The current object (for fluent API support)
     */
    public function setsfGuardUsers(PropelCollection $sfGuardUsers, PropelPDO $con = null)
    {
        $this->clearsfGuardUsers();
        $currentsfGuardUsers = $this->getsfGuardUsers();

        $this->sfGuardUsersScheduledForDeletion = $currentsfGuardUsers->diff($sfGuardUsers);

        foreach ($sfGuardUsers as $sfGuardUser) {
            if (!$currentsfGuardUsers->contains($sfGuardUser)) {
                $this->doAddsfGuardUser($sfGuardUser);
            }
        }

        $this->collsfGuardUsers = $sfGuardUsers;

        return $this;
    }

    /**
     * Gets the number of sfGuardUser objects related by a many-to-many relationship
     * to the current object by way of the sf_guard_user_group cross-reference table.
     *
     * @param Criteria $criteria Optional query object to filter the query
     * @param boolean $distinct Set to true to force count distinct
     * @param PropelPDO $con Optional connection object
     *
     * @return int the number of related sfGuardUser objects
     */
    public function countsfGuardUsers($criteria = null, $distinct = false, PropelPDO $con = null)
    {
        if (null === $this->collsfGuardUsers || null !== $criteria) {
            if ($this->isNew() && null === $this->collsfGuardUsers) {
                return 0;
            } else {
                $query = sfGuardUserQuery::create(null, $criteria);
                if ($distinct) {
                    $query->distinct();
                }
            ...
            return count($this->collsfGuardUsers);
        }
    }

    /**
     * Associate a sfGuardUser object to this object
     * through the sf_guard_user_group cross reference table.
     *
     * @param  sfGuardUser $sfGuardUser The sfGuardUserGroup object to relate
     * @return sfGuardGroup The current object (for fluent API support)
     */
    public function addsfGuardUser(sfGuardUser $sfGuardUser)
    {
        if ($this->collsfGuardUsers === null) {
            $this->initsfGuardUsers();
        }
        if (!$this->collsfGuardUsers->contains($sfGuardUser)) { // only add it if the **same** object is not already associated
            $this->doAddsfGuardUser($sfGuardUser);

            $this->collsfGuardUsers[]= $sfGuardUser;
        }
    ...
    /**
     * @param	sfGuardUser $sfGuardUser The sfGuardUser object to add.
     */
    protected function doAddsfGuardUser($sfGuardUser)
    {
        $sfGuardUserGroup = new sfGuardUserGroup();
        $sfGuardUserGroup->setsfGuardUser($sfGuardUser);
        $this->addsfGuardUserGroup($sfGuardUserGroup);
    }

    /**
     * Remove a sfGuardUser object to this object
     * through the sf_guard_user_group cross reference table.
     *
     * @param sfGuardUser $sfGuardUser The sfGuardUserGroup object to relate
     * @return sfGuardGroup The current object (for fluent API support)
     */
    public function removesfGuardUser(sfGuardUser $sfGuardUser)
    {
        if ($this->getsfGuardUsers()->contains($sfGuardUser)) {
            $this->collsfGuardUsers->remove($this->collsfGuardUsers->search($sfGuardUser));
            if (null === $this->sfGuardUsersScheduledForDeletion) {
                $this->sfGuardUsersScheduledForDeletion = clone $this->collsfGuardUsers;
                $this->sfGuardUsersScheduledForDeletion->clear();
            }
            $this->sfGuardUsersScheduledForDeletion[]= $sfGuardUser;
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collsfGuardUsers) {
                foreach ($this->collsfGuardUsers as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        ...
        $this->collsfGuardPermissions = null;
        if ($this->collsfGuardUsers instanceof PropelCollection) {
            $this->collsfGuardUsers->clearIterator();
        }
        $this->collsfGuardUsers = null;
    }
